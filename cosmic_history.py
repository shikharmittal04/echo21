...
def _eqns(Z,V,Ho,Om_m,Om_b,Tcmbo,Yp,falp,fX,fstar,Tmin_vir):
	...
	if Z>Zstar:
		eq2 = 2*Tk-Tk*eq1/(1+xHe(Yp)+xe)-Ecomp(Z,xe,Tk,Ho,Om_m,Tcmbo,Yp)-Ex2b(Z,xe,Tk,Tx,v_bx,Ho,Om_m, Tcmbo, Yp, fdm,mx,sigma45)
	else:
		eq2 = 2*Tk-Tk*eq1/(1+xHe(Yp)+xe)-Ecomp(Z,xe,Tk,Ho,Om_m,Tcmbo,Yp)-Ex(Z,xe,Ho,Om_m,Om_b,Tcmbo,fX,fstar,Tmin_vir)-Elya(Z,xe,Tk,Ho,Om_m,Om_b,Tcmbo,Yp,falp,fstar,Tmin_vir)-Ex2b(Z,xe,Tk,Tx,v_bx,Ho,Om_m, Tcmbo, Yp, fdm,mx,sigma45)
	
	#eq3 is (1+z)dTx/dz;
	eq3 = 2*Tx-Eb2x(Z,xe,Tk,Tx,v_bx,Ho,Om_m, Tcmbo, Yp, fdm,mx,sigma45)
	
	#eq2 is (1+z)dv_bx/dz;
	eq4 = v_bx + D(Z,xe,Tk,Tx,v_bx, Ho,Om_m,Om_b,Yp,fdm,mx,sigma45)/H(Z,Ho,Om_m, Tcmbo)
